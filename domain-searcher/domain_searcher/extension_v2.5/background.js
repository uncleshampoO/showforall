...
/**
 * Human-like random delay between page navigations.
 * @param {number} minSec
 * @param {number} maxSec
 */
function humanDelay(minSec = 3, maxSec = 7) {
...
async function runSearch(targetCount, sendProgress) {
    ...
    try {
        ...
        const allCandidates = [];
        let pageNum = 1;
        let emptyPagesInRow = 0; // RISK-2: track consecutive empty pages
        const maxPages = Math.min(Math.ceil(scrapeTarget / 25), 20); // Safety limit

        for (let p = 0; p < maxPages; p++) {
            if (activeSearch.cancelled) break;
            if (allCandidates.length >= scrapeTarget) break;

            // Coffee break every 5 pages
            if (p > 0 && p % 5 === 0) {
                sendProgress({ type: "status", message: "☕ Пауза для безопасности (60 сек)...", progress: 30 + (p / maxPages) * 40 });
                await humanDelay(50, 70); // 50-70 second break
            }
